...
function myfunction(position, startingMonth, startingDay, endMonth, endDay) {



  let datetime = "&start_date=2022-"+ startingMonth+"-"+startingDay+  " 09:30:00&end_date=2022-"+ endMonth+"-"+ endDay+" 15:59:00&" ;
  ...
  const url = "https://api.twelvedata.com/time_series?symbol=AMZN&interval=1min"+ datetime+"apikey=" + process.env.TWELVE;

  //console.log("check point" + position);
  console.log("check point" + url);
  //console.log("check");
  https.get(url, function(response) {
    ...
    response.on('end', () => {
      try {
        ...
        const creatTable = "CREATE TABLE IF NOT EXISTS "+ "amazon"+"( id INT NOT NULL AUTO_INCREMENT, timePoint DATETIME,minuteOpen double,minuteHigh double,minuteLow double,minuteClose double,minuteVolume double,PRIMARY KEY(id));"
...
app.listen(3000,function() {
  ...
  const loop = async () => {
    for (let i = 0; i < 30; i++ ) {
            ...
            console.log(myfunction(0,valueStartingMonth, valueStartingDay, valueEndMonth, valueEndDay));
              //console.log(startDay);
              //console.log(startingDate.getDate())
              //console.log("the month is " + startingDate.getMonth())
            // }
